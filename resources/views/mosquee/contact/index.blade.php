    ...
    <div class="card card-info">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('mosquee.contact.create', ['mosquee' => $mosquee->uuid]) }}" class="btn btn-secondary btn-sm">Tambah</a>
            </div>
        </div>
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <div></div>
                <form action="{{ route('mosquee.contact.index', ['mosquee' => $mosquee->uuid]) }}">
                    ...
                    <tbody>
                        @forelse ($paginate['data'] as $item)
                            <tr>
                                ...
                                <td>{{ $item['type'] }}</td>
                                <td>
                                    <a href="{{ route('mosquee.contact.edit', ['mosquee' => $mosquee->uuid, 'contact' => $item['id']]) }}" class="btn btn-warning">
                                        Ubah
                                    </a>
                                    <form action="{{ route('mosquee.contact.destroy', ['mosquee' => $mosquee->uuid, 'contact' => $item['id']]) }}" method="post">
                                        @csrf
                                        @method('delete')
