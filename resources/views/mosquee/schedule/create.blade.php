...
@section('title', 'Tambah Jadwal Masjid')
...
@section('breadcrumb')
    <ol class="breadcrumb float-sm-right">
        ...
        <li class="breadcrumb-item active">Tambah Jadwal Masjid</li>
    </ol>
@endsection
...
@section('content')
    <div class="card card-info">
        <div class="card-header">
            <h3 class="card-title">Tambah Dzikir</h3>
        </div>
        <div class="card-body">
            <form action="{{ route('mosquee.schedule.store', ['mosquee' => $mosquee->uuid]) }}" method="post" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-8">
                        <input id="speakers" type="text" class="form-control @error('speakers') is-invalid @enderror" name="speakers" value="{{ old('speakers') }}" required autocomplete="off">
                    ...
                    <div class="col-md-8">
                        <select id="day" class="form-control @error('day') is-invalid @enderror" name="day" required>
                            ...
                            @foreach (['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'] as $row)
                                <option value="{{ $row }}" @if (old('day') === $row) selected @endif>
                                    {{ $days[$row] }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-8">
                        <div class="form-group clearfix @error('type') is-invalid @enderror @error('type') is-invalid @enderror">
                            <div class="icheck-success d-inline mr-3">
                                <input type="radio" class="type-check" name="type" value="kajian" @if (old('type') === 'kajian') checked @else checked @endif id="checkboxSuccess1">
                                <label for="checkboxSuccess1">
                                    Kajian
                                </label>
                            </div>
                            <div class="icheck-success d-inline mr-3">
                                <input type="radio" class="type-check" name="type" value="tahsin" @if (old('type') === 'tahsin') checked @endif id="checkboxSuccess2">
                                <label for="checkboxSuccess2">
                                    Tahsin
                                </label>
                            </div>
                            <div class="icheck-success d-inline mr-3">
                                <input type="radio" class="type-check" name="type" value="tahfidz" @if (old('type') === 'tahfidz') checked @endif id="checkboxSuccess3">
                                <label for="checkboxSuccess3">
                                    Tahfidz
                                </label>
                            </div>
                            <div class="icheck-success d-inline">
                                <input type="radio" class="type-check" name="type" value="dauroh" @if (old('type') === 'dauroh') checked @endif id="checkboxSuccess4">
